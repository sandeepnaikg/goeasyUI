...
export default function ShoppingDetails() {
  ...
  const handleShare = async () => {
    if (!product) return;
    const text = `Check out ${product.name} on GoZy - ₹${product.price.toLocaleString()}\n${product.category}`;
    try {
    ...
    <div className="min-h-screen bg-white pb-24">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="bg-white rounded-3xl shadow-xl p-6 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-1">
              <ImageWithFallback src={product.image} alt={product.name} className="w-full h-72 object-cover rounded-2xl" />
            </div>
            <div className="md:col-span-2">
              <div className="flex items-start justify-between">
                <h1 className="text-2xl font-bold mb-2 text-gray-900">{product.name}</h1>
                <button onClick={() => setFavorite(`product:${product.id}`, !favorites[`product:${product.id}`])} className={`p-2 rounded-full bg-gradient-to-r ${favorites[`product:${product.id}`] ? 'from-rose-500 to-pink-600 text-white' : 'from-gray-100 to-gray-200 text-gray-800'}`} aria-label="Favorite">
                  <Heart className={`w-4 h-4 ${favorites[`product:${product.id}`] ? 'fill-white' : ''}`} />
                </button>
              </div>
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs px-2 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded">Price dropped recently</span>
              </div>
              {/* Variants */}
              <div className="mb-4 grid grid-cols-2 gap-3">
                <div>
                  <div className="text-xs font-semibold text-gray-700 mb-1">Color</div>
                  <div className="flex gap-2">
                    {['Black','Blue','Red'].map(c => (
                      <button key={c} onClick={()=> setVariant(v=> ({...v, color: c}))} className={`px-3 py-1.5 rounded-full border text-xs ${variant.color===c? 'bg-gray-900 text-white border-gray-900':'border-gray-300'}`}>{c}</button>
                    ))}
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex gap-2">
                    {['S','M','L','XL'].map(s => (
                      <button key={s} onClick={()=> setVariant(v=> ({...v, size: s}))} className={`px-3 py-1.5 rounded-full border text-xs ${variant.size===s? 'bg-gray-900 text-white border-gray-900':'border-gray-300'}`}>{s}</button>
                    ))}
                  </div>
                </div>
              </div>
              <p className="text-gray-700 mb-6">{product.description || 'Product details and features go here.'}</p>

              <div className="flex items-center space-x-4">
                <button onClick={handleAddToCart} className="px-6 py-3 bg-gradient-to-r from-orange-500 to-red-600 text-white rounded-xl font-bold">Add to Cart</button>
                <div className="text-xs text-gray-700">Delivery by <span className="font-semibold">{new Date(Date.now()+3*86400000).toISOString().slice(0,10)}</span></div>
                <button onClick={handleShare} className="px-4 py-2 bg-white border rounded-xl flex items-center space-x-2">
                ...
                <button onClick={() => {
                  ...
                  alerts.unshift({ id: `pa-${Date.now()}`, type: 'product', pid: product.id, price: product.price });
                  localStorage.setItem('priceAlerts', JSON.stringify(alerts.slice(0,100)));
                  pushNotification({ module: 'offers', title: 'Price alert set', message: `We will alert if ${product.name} drops in price` });
        ...
        <div className="bg-white rounded-3xl shadow p-6">
              ...
              {[5,4,3,2,1].map(st => (
                <div key={st} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-2 bg-gray-200 rounded">
                    <div className="h-2 bg-emerald-500 rounded" style={{ width: `${Math.max(5-st,1)*15}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Q&A */}
          <div className="mt-6">
            <button onClick={()=> setShowQA(s=>!s)} className="text-sm underline text-teal-700">{showQA? 'Hide':'Show'} Q&A</button>
            {showQA && (
              <div className="mt-2 space-y-2 text-sm text-gray-700">
                <div>
                  <div className="font-medium">Q: Does it come with warranty?</div>
                  <div>A: Yes, one-year manufacturer warranty included.</div>
                </div>
                <div>
                  <div className="font-medium">Q: Is this compatible with iOS and Android?</div>
