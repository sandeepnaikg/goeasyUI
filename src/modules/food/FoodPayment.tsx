...
export default function FoodPayment() {
  ...
  useEffect(() => {
    const orderDetails = JSON.parse(localStorage.getItem('foodOrderDetails') || '{}');
  ...
  const handlePaymentSuccess = (details: { paymentMethod: string; discount: number; finalAmount: number; rewardsEarned: number }) => {
    try {
      ...
      const cart = cartRaw ? JSON.parse(cartRaw) : { items: {}, restaurant: null };
      const order = {
        id: `FORD-${Date.now()}`,
        items: cart.items,
      ...
      hist.unshift(order);
      localStorage.setItem('foodOrderHistory', JSON.stringify(hist.slice(0,50)));
      // add to recently viewed/booked
      try {
        const rest = cart.restaurant || { name: 'Restaurant', image: undefined, id: 'rest' };
        addRecentlyViewed({ id: rest.id || `rest-${Date.now()}`, type: 'food', title: rest.name || 'Restaurant', image: rest.image });
      } catch { /* ignore */ }
      // wallet & rewards
  ...
  const tx = { id: `tx-${Date.now()}`, walletId: wallet.id, amount: details.finalAmount, type: 'debit' as const, category: 'food' as const, description: `Food Order ${order.id}`, method: 'wallet' as const, status: 'success' as const, toOrFrom: (cart.restaurant?.name) || 'Restaurant', referenceId: 'UTR'+Math.random().toString(36).slice(2,10).toUpperCase(), createdAt: new Date() };
      ...
      if (details.rewardsEarned && details.rewardsEarned > 0) {
        const act = { id: `ra-${Date.now()}`, source: 'food' as const, description: `Order ${order.id}`, points: details.rewardsEarned, createdAt: new Date() };
  ...
  return (
    <PaymentPage amount={totalAmount} onSuccess={handlePaymentSuccess} />
  );
}
