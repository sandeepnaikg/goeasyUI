...
export default function WalletHome() {
  ...
  const [selected, setSelected] = useState<
    (typeof walletTransactions)[number] | null
  >(null);
  ...
  const [settleBankAccount, setSettleBankAccount] =
  ...
  useEffect(() => {
    try {
      localStorage.setItem("walletVouchers", JSON.stringify(vouchers));
    } catch {
      /* ignore */
    }
  }, [vouchers]);
  ...
  const handleRedeem = () => {
    ...
    // celebrate wallet credit from rewards
    setTimeout(
      () =>
  ...
  const sendMoney = () => {
    ...
    const desc =
      sendMethod === "upi"
        ? `Sent to ${sendTo}`
        : `Bank transfer ${bankAccount} / ${bankIfsc}`;
    const tx = {
      id: "tx" + Date.now(),
      walletId: wallet.id,
      amount: sendAmount,
      ...
      description: desc,
      method: sendMethod,
      toOrFrom: sendMethod === "upi" ? sendTo : `${bankAccount} / ${bankIfsc}`,
      status: "success" as const,
      referenceId:
        "UTR" + Math.random().toString(36).slice(2, 10).toUpperCase(),
      createdAt: new Date(),
  ...
  const totalEarned = rewardActivities
    .filter((r) => r.points > 0)
    .reduce((s, r) => s + r.points, 0);
  const totalRedeemed = rewardActivities
    .filter((r) => r.points < 0)
    .reduce((s, r) => s + Math.abs(r.points), 0);
  const daysLeft = (() => {
    if (!payLater?.dueDate) return null;
    const diff = Math.ceil(
      (new Date(payLater.dueDate).getTime() - Date.now()) /
        (1000 * 60 * 60 * 24)
    );
    return diff;
  })();
  ...
  const settleWith = (method: "wallet" | "upi" | "card" | "bank") => {
    ...
    if (method === "wallet") {
      ...
      if (wallet.balance < amount) {
        setSettleError(
          "Insufficient wallet balance for repayment. Choose UPI/Card/NetBanking."
        );
        return;
      }
      ...
      const tx = {
        ...
        type: "debit" as const,
        category: "transfer" as const,
        description: "Pay Later repayment",
        method: "wallet" as const,
        status: "success" as const,
        referenceId:
          "PLR" + Math.random().toString(36).slice(2, 10).toUpperCase(),
        createdAt: new Date(),
      };
      const list = [tx, ...walletTransactions];
      setWalletTransactions(list);
    ...
    const updated = {
      ...payLater,
      used: Math.max(0, payLater.used - amount),
      dueAmount: Math.max(0, payLater.dueAmount - amount),
    };
    setPayLater(updated);
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <div className="mb-6 rounded-2xl p-4 bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-100">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="text-sm text-gray-800">
                <span className="font-semibold">Pay Later</span> • Limit ₹
                {payLater.limit.toLocaleString()} • Due ₹
                {payLater.dueAmount.toLocaleString()} • Due on{" "}
                {payLater.dueDate ? formatDate(payLater.dueDate) : "—"}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div className="bg-gradient-to-br from-teal-500 to-cyan-600 rounded-3xl shadow-2xl p-8 text-white">
            ...
            <div className="flex space-x-3 text-sm">
              <button
                ...
                className="flex-1 py-3 bg-white text-teal-700 rounded-xl font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center space-x-2 shine"
              >
              ...
              <button
                ...
                className="flex-1 py-3 bg-white/20 backdrop-blur-sm text-white rounded-xl font-semibold hover:bg-white/30 transition-colors flex items-center justify-center space-x-2"
              >
          ...
          <div className="bg-gradient-to-br from-amber-400 to-orange-500 rounded-3xl shadow-2xl p-8 text-white">
            ...
            <div
              ...
              className="text-5xl font-bold mb-4 cursor-pointer group"
            >
              {rewards?.points}{" "}
              <span className="text-base align-top font-medium group-hover:text-white/80">
                pts
              </span>
            </div>
            ...
            <button
              ...
              className="w-full py-3 bg-white text-orange-600 rounded-xl font-semibold hover:bg-gray-50 transition-colors flex items-center justify-center space-x-2"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <button
            ...
            className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-all text-left relative overflow-hidden group"
          >
            ...
            <p className="text-sm text-gray-600">
              {paymentCards.length} saved • manage methods
            </p>
            <div className="absolute inset-0 opacity-0 group-hover:opacity-10 bg-gradient-to-br from-brand-600 to-brand-500 transition-opacity" />
          </button>

          <button
            onClick={shareReferral}
            className="bg-white rounded-2xl shadow-lg p-6 hover:shadow-xl transition-all text-left group relative overflow-hidden"
          >
          ...
          >
            <div className="w-12 h-12 bg-brand-100 rounded-xl flex items-center justify-center mb-4">
              <TicketPercent className="w-6 h-6 text-brand-600" />
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white rounded-2xl shadow-lg p-6 md:col-span-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-xs text-gray-600">
                  Limit ₹{payLater.limit.toLocaleString()}
                </div>
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div className="p-4 rounded-xl bg-brand-50 border border-brand-100">
                  ...
                  <div className="text-xl font-bold text-brand-700">
                    ₹{payLater.used.toLocaleString()}
                  </div>
                </div>
                <div className="p-4 rounded-xl bg-amber-50 border border-amber-100">
                  <div className="text-xs text-gray-500">Due</div>
                  <div className="text-xl font-bold text-amber-700">
                    ₹{payLater.dueAmount.toLocaleString()}
                  </div>
                </div>
                <div className="p-4 rounded-xl bg-indigo-50 border border-indigo-100">
                  <div className="text-xs text-gray-500">Due Date</div>
                  <div className="text-sm font-semibold text-indigo-700 flex items-center gap-1">
              ...
              {payLater.dueAmount > 0 && (
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <button
                    onClick={settlePayLater}
                    className="px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 shine"
                  >
                    Pay Now
                  </button>
                </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 md:col-span-3">
              ...
              <div className="space-y-2">
                {walletTransactions
                  .filter((t) => t.description.includes("Pay Later repayment"))
                  .slice(0, 6)
                  .map((t) => (
                    <div
                      key={t.id}
                      className="flex items-center justify-between text-sm border-2 border-gray-100 rounded-xl p-3"
                    >
                ...
                {walletTransactions.filter((t) =>
                  t.description.includes("Pay Later repayment")
                ).length === 0 && (
        ...
        <div className="bg-white rounded-3xl shadow-xl p-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <button
              onClick={() => setCurrentPage("wallet-transactions")}
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              {rewardActivities
                ...
                .map((act) => (
                  <span
                    key={act.id}
                    className="px-2 py-1 rounded-full bg-red-50 border border-red-200 text-red-700"
                  >
                    {act.description} • {Math.abs(act.points)} pts
                  </span>
                ))}
          ...
          <div className="space-y-4">
            {sortedTransactions.map((transaction) => (
              <button
                ...
                className="w-full flex items-center justify-between p-4 border-2 border-gray-100 rounded-xl hover:border-brand-300 transition-colors text-left"
              >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-md bg-white rounded-2xl p-6 shadow-2xl relative">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gradient-to-r from-brand-50 to-brand-50 border border-brand-100 rounded-xl p-4 flex items-center space-x-3">
                  <WalletCards className="w-6 h-6 text-brand-600" />
                  <p className="text-sm text-brand-700">
                    Money will be added instantly to your GOZY Wallet.
                  </p>
                </div>
                <button
                  onClick={addMoney}
                  className="w-full py-3 rounded-xl btn-brand shine"
                >
                  Add ₹{addAmount}
                </button>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
            <div className="w-full max-w-md bg-white rounded-2xl p-6 shadow-2xl relative">
              ...
              <h3 className="text-xl font-bold mb-4 flex items-center gap-2">
                <TicketPercent className="w-5 h-5 text-brand-600" /> Vouchers
              </h3>
              <div className="space-y-3">
                {vouchers.map((v) => (
                  <div
                    key={v.code}
                    className="p-3 border-2 border-gray-100 rounded-xl flex items-center justify-between"
                  >
                    <div>
                      <div className="font-semibold text-gray-900">
                        {v.code} • {v.label}
                      ...
                      <div className="text-xs text-gray-600">
                        Min ₹{v.min} • Expires {formatDate(v.expiry)}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                      ...
                      <button
                        onClick={() => {
                          localStorage.setItem("selectedOfferCode", v.code);
                          setShowVouchers(false);
                        }}
                        className="px-2 py-1 rounded-lg bg-brand-600 text-white text-xs hover:bg-brand-700"
                      >
                        Apply
                      </button>
                    </div>
              ...
              <div className="space-y-4">
                ...
                {sendMethod === "upi" ? (
                  <input
                    ...
                    placeholder="upi@bank or phone/email"
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none"
                  />
                ) : (
                  <div className="space-y-3">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-3 py-3 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none"
                      />
                      {showIfscList && (
                        <div className="absolute z-10 mt-1 w-full bg-white border rounded-xl shadow">
                          {[
                            ...
                            .filter((b) =>
                              b.includes(bankIfsc.toUpperCase().slice(0, 4))
                            )
                            .slice(0, 5)
                            .map((code) => (
                              <button
                ...
                />
                <button
                  disabled={!!wallet && sendAmount > wallet.balance}
                  onClick={sendMoney}
                  className="w-full py-3 rounded-xl btn-brand shine disabled:opacity-50 flex items-center justify-center space-x-2"
                >
                  {sendMethod === "bank" ? (
                ...
                {wallet && sendAmount > wallet.balance && (
            ...
            <div className="w-full max-w-2xl bg-white rounded-2xl p-6 shadow-2xl relative">
              ...
              <div className="grid gap-4 mb-8">
                {paymentCards.map((c) => (
                  <div
                    key={c.id}
                    className="p-5 rounded-2xl bg-gradient-to-r from-slate-800 to-slate-700 text-white shadow-lg relative overflow-hidden"
                  >
              ...
              <div className="border-t pt-6">
                ...
                <div className="grid md:grid-cols-3 gap-3 mb-4">
                  <input
                    ...
                    className="px-3 py-2 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none md:col-span-2"
                  />
                  <input
                    ...
                    placeholder="MM/YY"
                    className="px-3 py-2 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none"
                  />
                  <input
                    value={newCard.name}
                    onChange={(e) =>
                    ...
                    placeholder="Name"
                    className="px-3 py-2 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none md:col-span-3"
                  />
                </div>
                <button
                  onClick={addCard}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm overflow-y-auto py-10">
            <div className="w-full max-w-xl bg-white rounded-2xl p-6 shadow-2xl relative">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Track how you earned and spent your points.
              </p>
              ...
              <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
                {rewardActivities.map((act) => (
                  <div
                    key={act.id}
                    className="flex items-center justify-between p-3 border-2 border-gray-100 rounded-xl hover:border-orange-200 transition-colors"
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-500">
                        {formatDate(act.createdAt)} • {act.source}
                      </div>
                    </div>
                    <div
                      className={`text-sm font-bold ${
                        act.points > 0 ? "text-green-600" : "text-red-600"
                      }`}
                    >
                      {act.points > 0 ? "+" : ""}
                      {act.points}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    value={settleAmount}
                    onChange={(e) =>
                      setSettleAmount(Number(e.target.value) || 0)
                    }
                    min={1}
                    max={payLater.dueAmount}
                    className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none"
                  />
                  <div className="flex gap-2 mt-2">
                    {(() => {
                      const minDue = Math.max(
                        500,
                        Math.floor(payLater.dueAmount * 0.25)
                      );
                      return (
                        <>
                          <button
                            onClick={() => setSettleAmount(minDue)}
                            className="px-3 py-1.5 text-xs rounded-full border border-gray-300 hover:border-brand-500"
                          >
                            Min Due ₹{minDue}
                          </button>
                          <button
                            onClick={() => setSettleAmount(payLater.dueAmount)}
                            className="px-3 py-1.5 text-xs rounded-full border border-gray-300 hover:border-brand-500"
                          >
                            Full Due ₹{payLater.dueAmount}
                          </button>
                        </>
                      );
                    })()}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      ...
                      className={`flex items-center justify-center gap-2 px-4 py-3 border-2 rounded-xl ${
                        settleMethod === "upi"
                          ? "border-brand-600"
                          : "border-gray-200 hover:border-brand-500"
                      }`}
                    >
                      <Smartphone className="w-4 h-4 text-brand-600" /> UPI
                    </button>
                    <button
                    ...
                    >
                      <CreditCard className="w-4 h-4 text-brand-600" /> Card
                    </button>
                    <button
                    ...
                    >
                      <Wallet className="w-4 h-4 text-brand-600" /> Wallet
                    </button>
                  </div>
                </div>
                {settleMethod === "upi" && (
                  <div>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                      We’ll request a UPI collect for ₹{settleAmount || 0}.
                    </div>
                  </div>
                )}
                {settleMethod === "card" && (
                  <div>
                    ...
                    {paymentCards.length > 0 ? (
                      <select
                        value={settleCardId || ""}
                        onChange={(e) => setSettleCardId(e.target.value)}
                      ...
                      <div className="text-xs text-gray-500">
                        No saved cards. Add one in Linked Cards.
                      </div>
                    )}
                    <div className="mt-2 text-xs text-gray-500">
                      We’ll charge ₹{settleAmount || 0} to the selected card.
                    </div>
                  </div>
                )}
                {settleMethod === "bank" && (
                  <div className="grid gap-3">
                    <div>
                      ...
                      <input
                        value={settleBankIfsc}
                        onChange={(e) =>
                          setSettleBankIfsc(e.target.value.toUpperCase())
                        }
                        className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-brand-500 outline-none"
                      />
                    </div>
                    ...
                    <div className="text-xs text-gray-500">
                      You’ll be redirected to your bank to pay ₹
                      {settleAmount || 0}.
                    </div>
                  </div>
                )}
                {settleMethod === "wallet" && (
                  <div className="text-sm text-gray-700">
                    ...
                    {wallet && settleAmount > wallet.balance && (
                      <span className="text-red-600">
                        • insufficient for this amount
                      </span>
                    )}
                  </div>
                )}
                {settleError && (
                  <div className="text-sm text-red-600">{settleError}</div>
                )}
                <button
                  onClick={() => settleWith(settleMethod)}
                  className="w-full py-3 rounded-xl btn-brand shine text-white font-semibold disabled:opacity-50"
                  disabled={
                    settleAmount <= 0 ||
                    (settleMethod === "wallet" &&
                      !!wallet &&
                ...
                <div className="text-xs text-gray-500 text-center">
                  No convenience fee. Payment simulated for demo.
                </div>
              </div>
            </div>
          </div>
