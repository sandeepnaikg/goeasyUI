...
import { CheckCircle, Download, Share2, Plane, Gift, Bus, Train, Ticket } from 'lucide-react';
import { useApp } from '../../context/AppContext';

export default function TravelConfirmation() {
  const [bookingRef, setBookingRef] = useState('');
  type FlightItem = { airline?: string; flightNumber?: string; departure?: string; arrival?: string; duration?: string; price?: number };
  type BusItem = { operator?: string; busType?: string; departure?: string; arrival?: string; duration?: string; price?: number };
  type TrainItem = { operator?: string; trainNo?: string; class?: string; departure?: string; arrival?: string; duration?: string; price?: number };
  type MetroItem = { line?: string; departure?: string; arrival?: string; duration?: string; price?: number };
  type Item = FlightItem | BusItem | TrainItem | MetroItem;
  const [order, setOrder] = useState<{ id: string; type: 'flight'|'bus'|'train'|'metro'; item: Item; total: number } | null>(null);
  const [pnr, setPnr] = useState<string | null>(null);
  const [berths, setBerths] = useState<string[] | null>(null);
  const { setCurrentModule, setCurrentPage } = useApp();

  useEffect(() => {
    setBookingRef('GOZY' + Math.random().toString(36).substr(2, 9).toUpperCase());
    const last = localStorage.getItem('lastTravelOrder');
    if (last) setOrder(JSON.parse(last));
    try {
      const sel = JSON.parse(localStorage.getItem('selectedTravelSeats') || 'null') as { mode?: string; seats?: { label?: string }[] } | null;
      if (sel && sel.mode === 'train' && Array.isArray(sel.seats)) {
        const labels = sel.seats.map(s => s.label).filter(Boolean) as string[];
        setBerths(labels.length ? labels : null);
        // Generate an IRCTC-like PNR: 10 digits split 3-7
        const p = `${Math.floor(100 + Math.random()*900)}-${Math.floor(1000000 + Math.random()*9000000)}`;
        setPnr(p);
      }
    } catch {/* ignore */}
  }, []);
  ...
  // Basic helpers
  const icon = order?.type === 'bus' ? <Bus className="w-8 h-8 text-white"/> : order?.type === 'train' ? <Train className="w-8 h-8 text-white"/> : <Plane className="w-8 h-8 text-white"/>;
  let title = '';
  let sub = '';
  let dep: string | undefined; let arr: string | undefined; let dur: string | undefined;
  if (order?.type === 'bus') {
    const it = order.item as BusItem;
    title = it.operator || 'Bus';
    sub = it.busType || '';
    dep = it.departure; arr = it.arrival; dur = it.duration;
  ...
  const qrSrc = bookingRef ? `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(bookingRef)}` : '';
  ...
  <div className="min-h-screen bg-white">
      <div className="max-w-3xl mx-auto px-4 py-12">
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600">Your {order?.type} has been successfully booked</p>
        </div>

        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden mb-6">
          <div className="bg-gradient-to-r from-teal-500 to-cyan-600 p-8 text-white">
            <div className="text-center">
              <div className="text-sm font-semibold mb-2">Booking Reference</div>
              <div className="text-4xl font-bold tracking-wider">{bookingRef}</div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            <div className="bg-white rounded-2xl p-6 border-2 border-gray-200 mb-8 text-center">
              <div className="text-sm text-gray-600 font-semibold mb-2">Magic QR</div>
              <div className="text-xs text-gray-500 mb-3">Scan at gate/check-in</div>
              {qrSrc && (
                <div className="p-3 bg-white rounded-xl border-2 border-gray-200 inline-block">
                  <img src={qrSrc} alt={`QR for ${bookingRef}`} className="w-56 h-56 object-contain rounded-lg" />
                </div>
              )}
            </div>
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl p-6 border-2 border-amber-200">
              ...
              <p className="text-gray-700">
                Congratulations! You've earned <span className="font-bold text-orange-600">{Math.floor((amount || 0) * 0.1)} points</span> with this booking.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-2xl overflow-hidden border">
          ...
          />
          <div className="text-xs text-gray-500 p-2">Route / airport map (mocked)</div>
        </div>
      </div>
    </div>
  );
