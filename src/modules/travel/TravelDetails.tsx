...
export default function TravelDetails() {
  ...
  const [chosenFare, setChosenFare] = useState<"Basic" | "Value" | "Flex">(
    "Value"
  );
    ...
    return [
      {
        ...
        price: base,
        perks: ["7kg cabin bag"],
        rules: ["No free changes", "No refund on cancellation"],
      },
      {
        family: "Value" as const,
        price: Math.max(0, base + 800),
        perks: ["7kg cabin bag", "15kg check-in"],
        rules: ["1 date change (₹500)", "Partial refund"],
      },
      {
        family: "Flex" as const,
        price: Math.max(0, base + 1800),
        perks: ["7kg cabin bag", "25kg check-in", "Preferred seat"],
        rules: ["Free date change ≥24h", "Full refund ≥48h"],
      },
    ];
  }, [flight]);
  ...
  useEffect(() => {
    ...
    if (data) {
      ...
      if (
        f &&
        typeof f.chosenFare === "string" &&
        ["Basic", "Value", "Flex"].includes(f.chosenFare)
      ) {
  ...
  const handleBook = () => {
    ...
    try {
      ...
      const fares = fareFamilies.map((f) => ({
        family: f.family,
        price: f.price,
      }));
  ...
  <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="bg-white rounded-2xl shadow p-6">
            <ErrorBlock
              ...
              message="Please pick a flight from results to view details and continue booking."
              action={
                <button
  ...
  const currentFare =
    fareFamilies.find((f) => f.family === chosenFare) || fareFamilies[1];
          ...
          <div className="p-8">
            ...
            <div className="mb-8">
              <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                <BadgePercent className="w-5 h-5 text-teal-600" /> Choose Fare
                Family
              </h3>
            ...
            <div className="mb-6">
              <div className="flex flex-wrap gap-2 items-center text-xs">
                ...
                <span className="px-2 py-1 rounded-full bg-gray-100 border text-gray-700">
                  Reschedule fee {chosenFare === "Flex" ? "Free ≥24h" : "₹500"}
                </span>
              </div>
              <div className="mt-2 text-xs text-gray-500 flex items-center gap-1">
                <Info className="w-4 h-4 text-gray-400" /> Final payable shown
                before payment.
              </div>
            </div>
            ...
            <div className="bg-teal-50 border border-teal-200 rounded-2xl p-6 mb-8">
              ...
              <p className="text-gray-700">
                Book this flight and earn{" "}
                <span className="font-bold text-teal-600">
                  450 reward points
                </span>{" "}
                (₹450 value) that you can use on your next booking!
              </p>
            </div>
              ...
              <div>
                ...
                {offerPreview && (
                  <div className="mt-2 text-sm text-gray-700">
                    <span className="inline-block bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-0.5 rounded">
                      {offerPreview.code}
                    </span>
                    <span className="ml-2">
                      −₹{offerPreview.discount} • New total:{" "}
                      <span className="font-semibold text-gray-900">
                        ₹{offerPreview.newTotal.toLocaleString()}
                      </span>
                    </span>
                  </div>
                )}
