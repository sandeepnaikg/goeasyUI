...
export default function PaymentPage({ amount, onSuccess }: PaymentPageProps) {
  ...
  const selectedUpi = useMemo(
    () => upiHandles.find((u) => u.id === selectedUpiId) || null,
    [upiHandles, selectedUpiId]
  );
  const [upiInput, setUpiInput] = useState<string>(selectedUpi?.handle || "");
  const [setCurrentUpiAsDefault, setSetCurrentUpiAsDefault] =
    useState<boolean>(false);
  useEffect(() => {
    setUpiInput(selectedUpi?.handle || "");
  }, [selectedUpi?.handle]);
  // Configurable rules
  const offerConfig = {
  ...
  const offers: Offer[] = useMemo(
    () => [
      {
        code: "FIRST100",
        label: "Flat ₹100 off for first order",
        terms: "Valid once per user. No minimum amount.",
        eligibleMethods: ["any"],
        stackWithWallet: true,
      },
      {
        code: "GOZY50",
        label: "Flat ₹50 off",
        terms: "No minimum amount.",
        eligibleMethods: ["any"],
        stackWithWallet: true,
      },
      {
        code: "WALLET100",
        label: "₹100 wallet cashback",
        terms: "Min order ₹499. Pay using GOZY Wallet.",
        eligibleMethods: ["wallet"],
        minAmount: 499,
      },
      {
        code: "GOFLY300",
        label: "Flights: ₹300 off",
        terms: "Min fare ₹2500.",
        eligibleMethods: ["any"],
        minAmount: 2500,
        stackWithWallet: true,
      ...
      {
        code: "HDFC10",
        label: "HDFC 10% on Card",
        terms: "Min ₹2999. Applicable only on cards.",
        eligibleMethods: ["card"],
        stackWithWallet: false,
      },
    ],
    []
  );
  ...
  const getDiscountFor = (
    ...
    if (code === "GOZY50") computed = 50;
      ...
      const used = (() => {
        try {
          return localStorage.getItem("firstOrderUsed") === "true";
        } catch {
          return false;
        }
      })();
      ...
      if (amt >= 499) computed = 100;
      else message = "Min order ₹499 required";
      if (method !== "wallet") message = "Pay with Wallet to avail this offer";
    } else if (code === "GOFLY300") {
      if (amt >= 2500) computed = 300;
      else message = "Min fare ₹2500 required";
    } else if (code === "BUS50") {
      if (amt >= 400) computed = 50;
      else message = "Min fare ₹400 required";
    }
    // Percentage with cap
    ...
    // Bank/card offer
      ...
      if (amt >= 2999 && method === "card") {
        ...
        message =
          method !== "card"
            ? "Use Credit/Debit Card to avail this offer"
            : "Min amount ₹2999 required";
      }
    } else {
  ...
  const applyPromo = () => {
    ...
    if (message) {
      ...
      try {
        localStorage.setItem("lastAppliedCodes", codesApplied.join("+"));
      } catch {
        /* ignore */
      }
    } else {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const { discount: computed, message } = getDiscountFor(
        code,
        amount,
        paymentMethod
      );
    ...
    if (last && !appliedCode) {
      ...
      for (const p of parts) {
        const { discount: d, message } = getDiscountFor(
          p,
          amount,
          paymentMethod
        );
  ...
  const getBestOffer = useCallback(
    (
      ...
      for (const c of allCodes) {
        ...
        if (d > bestSingle.discount)
          bestSingle = { code: c as string, discount: d };
      }
      if (method !== "wallet") {
        return {
          codes: bestSingle.code ? [bestSingle.code] : [],
          discount: bestSingle.discount,
        };
      }
      ...
      if (walletOnly > 0) {
        for (const c of allCodes) {
          ...
          if (d > 0) {
            ...
            if (sum > bestStack.discount)
              bestStack = { codes: [c as string, "WALLET100"], discount: sum };
          }
        }
      }
      // Compare
  ...
  useEffect(() => {
    ...
    if (best.codes.length > 0 && best.discount > 0) {
      ...
      if (
        ...
        setShowCouponPaper(true);
        setPromoMessage(
          `Best offer applied: ${codeStr} • You saved ₹${best.discount}`
        );
        setAutoApplied(true);
        setStackedWallet(
          best.codes.includes("WALLET100") && best.codes.length > 1
        );
        setTimeout(() => {
          setPromoMessage(null);
          setShowCouponPaper(false);
  ...
  const alternativeOffers = useMemo(() => {
    ...
    return allCodes
      .map((c) => {
        ...
        return {
          code: c as string,
          d: getDiscountFor(c, amount, paymentMethod).discount,
        };
      })
    ...
    appliedCode,
    allCodes,
    offerConfig.stackingCapPct,
  ]);

  const finalAmount = amount - discount;
  const rewardsEarned = Math.floor(finalAmount * 0.1);
  const plAvail = payLater?.enabled
    ? Math.max(0, (payLater.limit || 0) - (payLater.used || 0))
    : 0;
  ...
  const discountBreakdown = useMemo(() => {
    ...
    const items = parts.map((c) => ({
      code: c,
      value: getDiscountFor(c, amount, paymentMethod).discount,
    }));
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="mb-4 bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-3 text-sm">
          Tip: Your welcome bonus has been credited to Wallet and Rewards. Try
          applying an offer code below for extra savings.
        </div>
        <div className="mb-4">
          <TrustBadges context="payment" />
        </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-700">
                    {appliedCode} • You saved ₹{discount}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-3xl shadow-xl p-6 w-full">
              ...
              >
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-600">
                      Use GOZY Pay Later, clear dues next month
                    </div>
                  </div>
                </div>
                <input
            ...
            {paymentMethod === "upi" && (
              <div className="mb-6">
                ...
                <label className="mt-2 inline-flex items-center gap-2 text-xs text-gray-700">
                  <input
                    type="checkbox"
                    checked={setCurrentUpiAsDefault}
                    onChange={(e) =>
                      setSetCurrentUpiAsDefault(e.target.checked)
                    }
                  />
            ...
            {rewards && rewards.points >= 100 && (
              <div className="mb-4 flex items-center justify-between bg-indigo-50 border border-indigo-200 rounded-xl p-3 text-sm">
                <div>
                  Use your points to save now: {rewards.points} pts available
                  (100 pts = ₹1)
                </div>
                <button
                  onClick={() => {
                    ...
                    localStorage.setItem(
                      "rewards",
                      JSON.stringify({ ...rewards, points: newPts })
                    );
            ...
            <button
              ...
              onClick={() => {
                ...
                setTimeout(() => {
                  ...
                  if (paymentMethod === "paylater") {
                    ...
                    setPayLaterNote(
                      `Added to Pay Later. Bill due by ${dueDate}.`
                    );
                    setTimeout(() => setPayLaterNote(null), 2400);
                  }
                  // Silently set default method to the last used method (no confirm dialogs)
                  try {
                    const currentDefault = localStorage.getItem(
                      "defaultPaymentMethod"
                    ) as "wallet" | "upi" | "card" | "paylater" | null;
                    ...
                    if (
                      paymentMethod === "upi" &&
                      setCurrentUpiAsDefault &&
                      upiInput &&
                      /^[a-zA-Z0-9._-]{2,}@[a-zA-Z]{2,}$/.test(upiInput)
                    ) {
                      // create or update existing list entry
                      const existing = upiHandles.find(
                        (u) => u.handle === upiInput
                      );
                      if (existing) {
              ...
              className={`w-full py-4 ${
                isPaying ? "bg-gray-400 cursor-not-allowed" : "btn-brand shine"
              } text-white text-xl font-bold rounded-xl transition-all shadow-lg hover:shadow-xl`}
            >
              {isPaying
                ? "Processing…"
            ...
            {payLaterNote && (
              <div className="mt-2 text-xs bg-blue-50 border border-blue-200 text-blue-800 px-3 py-2 rounded">
                {payLaterNote}
              </div>
            )}
            ...
            <div ref={promoSectionRef} className="mt-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button
                  ...
                  className="text-xs underline text-brand-700 hover:text-brand-800"
                >
                  View all offers
                </button>
              </div>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border-2 border-gray-200 rounded-xl focus:border-brand-500 focus:outline-none"
                />
                <button
                  onClick={applyPromo}
                  className="px-5 py-2 btn-brand shine rounded-xl font-semibold"
                >
                  Apply
                </button>
              </div>
                    ...
                    {alternativeOffers.map((a) => (
                      <button
                        ...
                        onClick={() => {
                          setPromoCode(a.code);
                          setPromoMessage(
                            `Ready to apply ${a.code} • Save up to ₹${a.d}`
                          );
                          setTimeout(() => setPromoMessage(null), 1800);
                        }}
                        className="px-2 py-1 rounded-full border border-gray-200 hover:border-brand-400 hover:text-brand-700"
                      >
                        {a.code}
                      </button>
                    ))}
              ...
              {!privacy?.personalizedOffers && (
                <div className="mt-2 text-[11px] text-gray-500">
                  Personalization is off — auto-apply is disabled. You can still
                  apply any code manually.
                </div>
              )}
              {promoMessage && (
                <div
              ...
              {discount > 0 && (
                <div className="mt-2 flex items-center space-x-2 text-green-600 text-sm">
                  ...
                  <span>
                    ...
                    {stackedWallet ? " (stacked with WALLET100)" : ""}!
                  </span>
                </div>
              )}
            </div>
            {paymentMethod === "paylater" && (
              <div
                ...
                } mb-4 px-3 py-2 rounded border text-xs`}
              >
                {plBlocked ? (
                  <>
                    Available Pay Later limit is {plAvail.toLocaleString()}.
                    Please reduce amount or choose another method.
                  </>
                ) : (
            ...
            <div className="space-y-3">
              ...
              {paymentMethod !== "card" && hdfcPotential.extra > 0 && (
                <div className="flex items-center justify-between text-xs bg-indigo-50 border border-indigo-200 text-indigo-800 px-2 py-1 rounded">
                  <span>
                    Switch to Card for extra ₹{hdfcPotential.extra} off (HDFC10)
                  </span>
                  <button
                    className="underline"
                    onClick={() => {
                  ...
                  <div className="flex items-center justify-between text-green-700">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        ...
                        title={`When paying with Wallet, GOZY may stack WALLET100 with one other offer. Total savings are capped at ${
                          offerConfig.stackingCapPct * 100
                        }% of order value.`}
                      >
                        i
                      </span>
              ...
              {paymentMethod === "paylater" && (
                <div className="text-xs text-gray-600">
                  Will be added to your Pay Later bill (due next month).
                </div>
              )}
            </div>
          </div>
        ...
        >
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-5">
              ...
              {offers.map((o) => {
                const preview = (() => {
                  ...
                  if (
                    ...
                    return Math.max(
                      single,
                      Math.min(single + wal, stackingCap)
                    );
                  }
                  return single;
                })();
                const disabled =
                  o.eligibleMethods.indexOf("any") === -1 &&
                  o.eligibleMethods.indexOf(paymentMethod) === -1;
                return (
                  <div
                    key={o.code}
            ...
            <div className="mt-3 text-xs text-gray-500">
              Note: Wallet cashback (WALLET100) can stack with one other offer
              when paying via GOZY Wallet. Total savings capped at 40% of the
              order value. See{" "}
              <a
                href="#promo-terms"
                onClick={(e) => {
                  e.preventDefault();
                  localStorage.setItem("navigateTo", "terms#promo-terms");
                  window.dispatchEvent(new Event("nav-anchor"));
                }}
