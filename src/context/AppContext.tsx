...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [upiHandles, setUpiHandles] = useState<UpiHandle[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("upiHandles") || "[]");
    } catch {
      return [];
    }
  });
  ...
  >(() => {
    try {
      return (
        (localStorage.getItem("defaultPaymentMethod") as
          | "wallet"
          | "upi"
          | "card"
          | "paylater") || "wallet"
  ...
  const [defaultCardId, setDefaultCardId] = useState<string | null>(() => {
    try {
      return localStorage.getItem("defaultCardId");
    } catch {
      return null;
    }
  });
  const [defaultUpiId, setDefaultUpiId] = useState<string | null>(() => {
    try {
      return localStorage.getItem("defaultUpiId");
    } catch {
      return null;
    }
  });
  ...
  const [defaultAddressId, setDefaultAddressId] = useState<string | null>(
    () => {
      try {
        return localStorage.getItem("defaultAddressId");
      } catch {
        return null;
      }
    }
  ...
  const [referral, setReferral] = useState<ReferralState>(() => {
    try {
      ...
      if (raw) return JSON.parse(raw);
      return {
        myCode: "GOZY-" + Math.random().toString(36).slice(2, 7).toUpperCase(),
        credits: 0,
      };
    } catch {
      return {
  ...
  const [payLater, setPayLater] = useState<PayLaterState>(() => {
    try {
      ...
      return raw
        ? JSON.parse(raw)
        : { enabled: true, limit: 5000, used: 0, dueAmount: 0 };
    } catch {
      return { enabled: true, limit: 5000, used: 0, dueAmount: 0 };
    }
  });
  ...
  useEffect(() => {
    try {
      ...
      if (r) setRewards(JSON.parse(r));
        ...
        const defaultRewards: RewardsPoints = {
          id: "r1",
          userId: "1",
          points: 100,
          tier: "bronze",
        ...
        const seed: WalletTransaction[] = [
          {
            ...
            method: "gateway",
            status: "success",
            referenceId: "UTRFL123456",
            toOrFrom: "GOZY TRAVEL",
            createdAt: new Date("2025-10-03T10:35:00"),
          },
          {
            id: "t2",
            walletId: "w1",
            ...
            method: "cashback",
            status: "success",
            referenceId: "CBK987654",
            toOrFrom: "GOZY REWARDS",
            createdAt: new Date("2025-10-03T10:36:00"),
          },
          {
            id: "t3",
            walletId: "w1",
            ...
            method: "wallet",
            status: "success",
            referenceId: "UTRFD432198",
            toOrFrom: "Pizza Hub",
            createdAt: new Date("2025-10-02T19:15:00"),
          },
          {
            id: "t4",
            walletId: "w1",
            ...
            method: "refund",
            status: "success",
            referenceId: "REF112233",
            toOrFrom: "GOZY REFUND",
            createdAt: new Date("2025-10-01T08:20:00"),
          },
        ];
    ...
    try {
      ...
      if ((rewards.points || 0) > prev) {
        ...
        const notif = {
          ...
          title: "Points added",
          message: `You earned ${diff} pts!`,
        };
        setNotifications((prevN) =>
          [
            notif as unknown as import("../types").AppNotification,
            ...prevN,
          ].slice(0, 100)
        );
      }
      ...
      if (!prevId && currId) {
        ...
        if (!already) {
          ...
          setRewardActivities((prev) => {
            ...
            try {
              localStorage.setItem("rewardActivities", JSON.stringify(next));
            } catch {
              /* ignore */
            }
            return next;
          ...
          const notif = {
            ...
            module: "system" as const,
            title: "Welcome bonus",
            message: `You received ${bonus} points for joining!`,
          };
  ...
  useEffect(() => {
    try {
      localStorage.setItem("payLater", JSON.stringify(payLater));
    } catch {
      /* ignore */
    }
  }, [payLater]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem("upiHandles", JSON.stringify(upiHandles));
    } catch {
      /* ignore */
    }
  }, [upiHandles]);
  ...
  useEffect(() => {
    try {
      if (defaultCardId) localStorage.setItem("defaultCardId", defaultCardId);
      else localStorage.removeItem("defaultCardId");
    } catch {
      /* ignore */
    }
  }, [defaultCardId]);
  useEffect(() => {
    try {
      if (defaultUpiId) localStorage.setItem("defaultUpiId", defaultUpiId);
      else localStorage.removeItem("defaultUpiId");
    } catch {
      /* ignore */
    }
  }, [defaultUpiId]);
      ...
      value={{
        ...
        addRecentlyViewed: (v) => {
          setRecentlyViewed((prev) => {
            ...
            const deduped = [
              entry,
              ...prev.filter((p) => !(p.id === v.id && p.type === v.type)),
            ].slice(0, 50);
            try {
          ...
          setSessions((prev) =>
            prev
              .map((s) => ({
                ...s,
                current: s.current,
                lastActive: s.current ? new Date() : s.lastActive,
              }))
              .slice(0, 1)
          );
        },
